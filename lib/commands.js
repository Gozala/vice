...
function FocusEditor(env) {
  ...
  env.cli.cliView.element.addEventListener("keyup", focusEditor, true);
  return true;
}
...
var commands = exports.commands = {
  commandLine: {
    exec: function exec(env, params, request) {
      var cli = env.cli.cliView.element;
      env.editor.blur();
      cli.focus();
      // Yeap it's an ugly hack to put focus back to an editor.
      return exec.inited || (exec.inited = FocusEditor(env));
    }
  },
  start: {
